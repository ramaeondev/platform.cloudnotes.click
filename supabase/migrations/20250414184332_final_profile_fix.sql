-- Update the handle_new_user function to use truly unique usernames
...
AS $function$
BEGIN
  -- Create the user profile with correct column names and UUID-based username
  ...
  VALUES (
    ...
    COALESCE(new.raw_user_meta_data->>'last_name', 
      ...
      END),
    new.email,
    regexp_replace(new.email, '@.*$', '') || '_' || substring(gen_random_uuid()::text, 1, 8),
...
-- Also update the fallback function to use UUID for username
...
-- Update the RPC function to use UUID for username too
